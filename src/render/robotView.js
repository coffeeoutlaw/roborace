...
// Per-seat models (name -> assets/robots/<name>.fbx + <name>_basecolor.jpg).
// yaw: extra rotation so the model faces -Z (north) at yaw 0. height: target world
// height in tiles; hover: levitation above the floor.
// standX cancels FBXLoader's 90° axis tilt (stand upright). yaw turns the model so
// its front faces -Z (north) at rest: these Tripo meshes face +X (east), so +90°
// CCW about Y points them north.
// Each model is matched to the seat whose marker colour matches the model's own
// paint: you=blue→futuristic, crusher=red→cute, prudence=green→screw,
// turbo=yellow→classic. standX cancels FBXLoader's 90° tilt; yaw points front north.
const MODELS = {
  crusher: { name: 'cute', standX: -Math.PI / 2, yaw: Math.PI / 2, height: 0.85, hover: 0 },
  prudence: { name: 'screw', standX: -Math.PI / 2, yaw: Math.PI / 2, height: 0.8, hover: 0 },
  turbo: { name: 'classic', standX: -Math.PI / 2, yaw: Math.PI / 2, height: 0.85, hover: 0 },
  default: { name: 'futuristic', standX: -Math.PI / 2, yaw: Math.PI / 2, height: 0.95, hover: 0 },
};
...
export class RobotView {
  ...
  #loadModel(robot) {
    const spec = MODELS[robot.personality] || MODELS.default;
